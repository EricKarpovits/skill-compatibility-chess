...
class LeelaBoard:
    ...
    def lcz_features(self, board_stack_size = 8):
        ...
        if board_stack_size > 8 or board_stack_size < 1:
            raise AttributeError(f"Board histories longer than 8 or shorter than 1 not supported, {board_stack_size} given")
        for data in self.lcz_stack[-1:-(board_stack_size + 1):-1]:
            ...
            planes_yielded += 13
        empty_planes = [flat_planes[0] for _ in range((13 * board_stack_size)-planes_yielded)]
