...
def display_arr(arr, as_grid = False):
    ...
    for i in range(4):
        ...
        for k in range(2):
            ...
            if as_grid:
                ...
                row_vals.append(
                    (str(chess.svg.board(chess.Board(boar_arr_to_fen(board_arr, active_white = is_white % 2, no_reorder = True)))),
                     arr[i*26 + 12 + (13 * k)].mean() > 0,
                    ))
            is_white += 1
        if i < 1:
    ...
    return HTMLWrapper(f"""<table style="width: 600px">
    <tr>
    <th>Active Player</th>
    <th>Opponent</th>
    <th>Info</th>
...
def l_plt(arr):
    ...
    for i in range(4):
        for k in range(2):
            ap = arr[i*26 + (13 * k): i*26 + 12 + (13 * k)]
            board_arr = make_board_arr(ap)
            seaborn.heatmap(make_board_arr(ap), ax = axes[i,k], cbar=False, annot = annot_arr(board_arr))
...
def grid_plot(arr):
    ...
    for i in range(6):
        for j in range(2):
            axes[j, i].imshow(arr[i + (6 * j)], cmap='Greys', vmax = 1, vmin = -1, interpolation='nearest')
