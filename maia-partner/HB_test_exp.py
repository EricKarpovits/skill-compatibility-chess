...
class team:
    ...
    def play(self,board,leelaboard,bit):
        try:
            if(self.expector):
                ...
                for m in mvs:
                    if board.piece_at(chess.parse_square(m[0:2])) not in lmoves:
                        lmoves[board.piece_at(chess.parse_square(m[0:2]))]=[]
                        lprobas[board.piece_at(chess.parse_square(m[0:2]))]=[]
                    lmoves[board.piece_at(chess.parse_square(m[0:2]))].append(chess.Move.from_uci(m))
                    lprobas[board.piece_at(chess.parse_square(m[0:2]))].append(mvs[m])
                mp=-10000000
                pi=None
                #print(lmoves,lprobas)
                for piece in lmoves:
                    mvt4=  sorted(zip(lprobas[piece], lmoves[piece]))[-3:]
                    s=sum([mvt4[i][0] for i in range(len(mvt4))])
                    tval=0
                    for movep in mvt4:
                        move=movep[1]
                        tboard=board.copy()
                        tboard.push(move)
                        val=self.partner.analyze(tboard, noden=50)
                        tval+=val*movep[0]/s
                    ...
                    if(tval>mp):
                        ...
                        pi=piece
                #print(pi)
                return np.random.choice(lmoves[pi], p=np.array(lprobas[pi])/sum(lprobas[pi]))
                #return lmoves[pi][np.argmax(lprobas[pi])]
        ...
        #print(mv,brain,mvs,board, lmoves)
            return np.random.choice(lmoves, p=np.array(lprobas)/sum(lprobas))
            ...
            self.partner.reset(True)
            feny=board.fen()
            fenyl=feny.split('-')[0]
            print("reset",fenyl)
            tboard=chess.Board(fen=fenyl)
            return self.play(tboard,leelaboard,bit)
...
def play(white, black,plystring):
    ...
    while board.outcome() is None:
        numplies+=1
        if(t%2==0):
            result = white.play(board,leelaboard,plystring[t])
        else:
            result = black.play(board,leelaboard,plystring[t])
        board.push(result)
...
def worker(partner1_name, partner2_name, partner1_weights, partner2_weights,i,q,bitstring):
    ...
    partnery=agent(partner2_name,["lc0" , "--weights="+partner2_weights,"--backend-opts=gpu="+str((i%1))],1500)   
    team2=team(partnery,partnery,expector=True)
