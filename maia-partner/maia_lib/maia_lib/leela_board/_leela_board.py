...
class LeelaBoard:
    ...
    def __getattr__(self, attr):
        try:
            return getattr(self.pc_board, attr)
        except AttributeError:
            if attr in self.__dict__:
                return self.__dict__[attr]
            if not hasattr(self, "pc_board"):
    ...
    def __getstate__(self) -> dict:
        ...
        state["pc_board"] = self.pc_board.copy()
        return state
    ...
    def deserialize_features(cls, serialized):
        ...
        for history_idx in range(0, bytes_per_history * 8, bytes_per_history):
            ...
            if not side_to_move:
                # we're white
                planes = np.unpackbits(memoryview(plane_bytes))[::-1].reshape(12, 8, 8)[  # type: ignore
                    ::-1
                ]
            else:
                # We're black
    ...
    def lcz_features(self, board_stack_size: int = 8) -> np.ndarray:
        """
    ...
    def lcz_uci_to_idx(self, uci_list: typing.List[str]) -> typing.List[int]:
    ...
    def move_to_idx(self, uci_move: str) -> int:
        data = self.lcz_stack[-1]
        uci_to_idx_index = (data.us_ooo | data.us_oo) + 2 * data.side_to_move
        uci_idx_dct = _uci_to_idx[uci_to_idx_index]
        return uci_idx_dct[uci_move.rstrip("n")]

    @classmethod
    def compress_features(cls, features: np.ndarray) -> bytes:
        ...
        piece_plane_bytes = np.packbits(features_8[:-8]).tobytes()  # type: ignore
